          ...
          <div class="table">
            <template v-for="(subItem,subIndex) in item.list">
              <div class="tr">
                ...
                <div class="td">{{subItem.facilitator}}</div>
...
<script>
  ...
  export default {
    data() {
      return {
        tagName: '维权订单',
        listHeaderItems:['服务名称','服务商信息','支付时间','成交价格','服务状态']
      }
    },
    created(){
//      this.getServiceOrderList_Refund()
    },
    computed:{
      ...mapState({
        serviceOrderListTotalAmount: state => state.serviceOrderList.serviceOrderRefundListTotalAmount,
        serviceOrderList: state => state.serviceOrderList.serviceOrderListRefund
      }),
