          ...
          <div class="table">
            <template v-for="(subItem,subIndex) in item.list">
              <div class="tr">
                ...
                <div class="td">{{subItem.finishTime || '——'}}</div>
...
<script>
  ...
  export default {
    data() {
      return {
        tagName: '待评价订单',
        listHeaderItems:['服务名称','单价','支付时间','完成时间','服务状态']
      }
    },
    created(){
//      this.getServiceOrderList_Uncommented()  //访问接口
    },
    computed:{
      ...mapState({
        serviceOrderListTotalAmount: state => state.serviceOrderList.serviceOrderUncommentedListTotalAmount,
        serviceOrderList: state => state.serviceOrderList.serviceOrderListUncommented
      }),
