          ...
          <div class="table">
            <template v-for="(subItem,subIndex) in item.list">
              <div class="tr">
                ...
                <div class="td">{{subItem.orderTime}}</div>
                <div class="td">{{subItem.price}}</div>
                <div class="td">{{subItem.amount}}</div>
                <div class="td">{{subItem.price*subItem.amount}}</div>
...
<script>
  ...
  export default {
    data() {
      return {
        tagName: '未付款订单',
        listHeaderItems:['服务名称','下单时间','单价','数量','合计']
      }
    },
    created(){
//      this.getServiceOrderList_Unpayed()  //访问接口
    },
    computed:{
      ...mapState({
        serviceOrderListTotalAmount: state => state.serviceOrderList.serviceOrderUnpayedListTotalAmount,
        serviceOrderList: state => state.serviceOrderList.serviceOrderListUnpayed
      }),
